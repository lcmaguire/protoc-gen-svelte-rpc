<script>
    // @ts-nocheck
    /* this file was generated by protoc-gen-svelte-rpc do not edit */
    import ExtraEdit from "$lib/tutorial/ExtraEdit.svelte";
    import NestEdit from "$lib/tutorial/Example.NestEdit.svelte";
    import OperatingSystemEdit from "$lib/tutorial/OperatingSystemEdit.svelte";
    import { Example } from "$lib/gen/example_pb";
    export let message;
    if (message == null) {
        message = new Example();
    }
    function pushextraArray() {
        message.extra = message.extra.concat(undefined);
    }
    function removeextraArray(index) {
        message.extra.splice(index, 1);
        message.extra = message.extra;
    }

    function pushtagsCategoryArray() {
        message.tagsCategory = message.tagsCategory.concat(undefined);
    }
    function removetagsCategoryArray(index) {
        message.tagsCategory.splice(index, 1);
        message.tagsCategory = message.tagsCategory;
    }

    // any messages within oneof need to be initialized.
    function setuptreeOneof() {
        message.tree.value = undefined;
        message.tree.case = treeview;
    }
    let treeview;
    $: treeview, setuptreeOneof();
</script>

<!-- this file was generated by protoc-gen-svelte-rpc do not edit-->

<label for="message-name"> message.name </label> <br />
<input class="message-name" bind:value={message.name} />
<br />

<label for="message-displayName"> message.displayName </label> <br />
<input class="message-displayName" bind:value={message.displayName} />
<br />

<label for="message-active"> message.active </label> <br />
<input class="message-active" type="checkbox" bind:checked={message.active} />
<br />

<label for="message-count"> message.count </label> <br />
<input
    class="message-count"
    type="number"
    bind:value={message.count}
    min="0"
    step="1"
/>
<br />

<br />
<label for="message-extra"> message.extra </label> <br />

{#each message.extra as item, key}
    <ExtraEdit bind:message={item} />

    <button on:click={() => removeextraArray(key)}>
        Remove from message.extra</button
    ><br />
{/each}
<button on:click={pushextraArray}> Add new message.extra</button><br />

<br />
<label for="message-tagsCategory"> message.tagsCategory </label> <br />

{#each message.tagsCategory as item, key}
    <input class="message-tagsCategory" bind:value={item} />
    <br />
    <button on:click={() => removetagsCategoryArray(key)}>
        Remove from message.tagsCategory</button
    > <br />
{/each}
<button on:click={pushtagsCategoryArray}> Add new message.tagsCategory</button>
<br />

<NestEdit bind:message={message.nest} />

<label for="message-clean-select"> message.clean </label> <br />
<select class="message-clean-select" bind:value={message.clean}>
    <option value="CLEANLINESS_UNSPECIFIED">CLEANLINESS_UNSPECIFIED</option>
    <option value="CLEANLINESS_DISGUSTING">CLEANLINESS_DISGUSTING</option>
    <option value="CLEANLINESS_BAD">CLEANLINESS_BAD</option>
    <option value="CLEANLINESS_GOOD">CLEANLINESS_GOOD</option>
    <option value="CLEANLINESS_EXCELLENT">CLEANLINESS_EXCELLENT</option>
</select><br />

<label for="message-birdNest-select"> message.birdNest </label> <br />
<select class="message-birdNest-select" bind:value={message.birdNest}>
    <option value="BIRD_NEST_UNDEFINED">BIRD_NEST_UNDEFINED</option>
    <option value="BIRD_NEST_DESTROYED">BIRD_NEST_DESTROYED</option>
    <option value="BIRD_NEST_BUILT">BIRD_NEST_BUILT</option>
</select><br />

<OperatingSystemEdit bind:message={message.os} />

<label>
    use treeType for Example oneof ?
    <input type="radio" bind:group={treeview} value={"treeType"} /> <br />
</label>
{#if treeview == "treeType"}
    <label for="message-tree.value"> message.tree.value </label> <br />
    <input class="message-tree.value" bind:value={message.tree.value} />
    <br />
{/if}
<label>
    use bush for Example oneof ?
    <input type="radio" bind:group={treeview} value={"bush"} /> <br />
</label>
{#if treeview == "bush"}
    <label for="message-tree.value"> message.tree.value </label> <br />
    <input
        class="message-tree.value"
        type="checkbox"
        bind:checked={message.tree.value}
    />
    <br />
{/if}
